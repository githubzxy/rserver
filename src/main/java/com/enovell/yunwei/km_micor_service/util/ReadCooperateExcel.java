...
public class ReadCooperateExcel {
	//总行数
 ...
 * @throws ParseException 
   */
	private List<ConstructRepairDto> readExcelValue(Workbook wb) throws ParseException {
		...
		//获取第一行的内容作为计划日期
		String planTimeStr = sheet.getRow(0).getCell(0).getStringCellValue();
			...
			for (int c = 0; c < this.totalCells; c++) {
				...
				//将第一列的数据替换成第一行获取的时间
				 if (c == 1){
							try {
								//截取第一行数据的时间
								StringBuffer str =new StringBuffer(planTimeStr.substring(0, 10));
						...
						if(cell.getCellType() == HSSFCell.CELL_TYPE_NUMERIC){
							...
							String planNum = String.valueOf(df.format(cell.getNumericCellValue()));
							//将字符串的后两位（.0）去掉
