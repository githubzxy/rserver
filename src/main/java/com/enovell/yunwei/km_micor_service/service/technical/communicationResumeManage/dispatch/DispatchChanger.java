...
import com.enovell.yunwei.km_micor_service.util.communicationResumeManage.ReadExcelDispatchChanger;
 ...
 * 类描述:  调度交换机excel导入实现
 * 创建人：zhouxingyu
 * 创建时间：2019年6月19日 下午4:24:47
 * 修改人：zhouxingyu 
 * 修改时间：2019年6月19日 下午4:24:47   
 *
 */
@Service("dispatchChanger")
public class DispatchChanger implements DispatchService{
	...
	@Override
	 public ResultMsg importDispatchTrunk(String publicType, MultipartFile file, String userId, String orgId) {
		 ...
		 * @return 
		 */
		public Map<String,Object> checkNullRow(List<DispatchChangerDto> fdpList) {
	     ...
	     //检验每一必填列数据中是否有未填，默认为true
	   	 ...
	   	 boolean portConfNull =fdpList.stream().anyMatch(f->StringUtils.isNotBlank(f.getPortConf())==false);
	   	 boolean numOfPortUsedNull =fdpList.stream().anyMatch(f->StringUtils.isNotBlank(f.getNumOfPortUsed())==false);
	   	 boolean numOfDispatchNull =fdpList.stream().anyMatch(f->StringUtils.isNotBlank(f.getNumOfDispatch())==false);
	   	 boolean numOfPhoneNull =fdpList.stream().anyMatch(f->StringUtils.isNotBlank(f.getNumOfPhone())==false);
	   	...
	   	if(portConfNull==true) {
	   		map.put("isNull", false);
	   		map.put("ResultMsg", ResultMsg.getFailure("导入失败!请检查'2M端口配置容量'列是否有空白格"));
	   		return map; 
	   	 }
	   	if(numOfPortUsedNull==true) {
	   		map.put("isNull", false);
	   		map.put("ResultMsg", ResultMsg.getFailure("导入失败!请检查'2M端口实际使用数量'列是否有空白格"));
	   		return map; 
	   	 }
	   	if(numOfDispatchNull==true) {
	   		map.put("isNull", false);
	   		map.put("ResultMsg", ResultMsg.getFailure("导入失败!请检查'调度台数量'列是否有空白格"));
	   		return map; 
	   	 }
		if(numOfPhoneNull==true) {
	   		map.put("isNull", false);
		...
		public Map<String,Object> checkWorkshopAndWorkArea(List<DispatchChangerDto> fdpList) {
			 Map<String, Object> map = new HashMap<String, Object>();
	        //获取所有车间
...
//          Bson filter = getFilter(  workshop,  workArea,  deviceName,  publicType, deviceClass, deviceType, manufacturers);
	...
	public ResultMsg importDispatchDispatchChanger(String publicType, MultipartFile file, String userId, String orgId) {
    	ReadExcelDispatchChanger readExcel =new ReadExcelDispatchChanger();
    	List<DispatchChangerDto> fdpList = readExcel.getExcelInfo(file);
...
//		    		document.put("source", "1");
		    		...
		    		document.put("portConf", s.getPortConf()); 
		    		document.put("numOfPortUsed", s.getNumOfPortUsed()); 
		    		document.put("numOfDispatch", s.getNumOfDispatch()); 
		    		document.put("numOfPhone", s.getNumOfPhone()); 
