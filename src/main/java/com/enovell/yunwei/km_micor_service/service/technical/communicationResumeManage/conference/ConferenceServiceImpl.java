...
import com.enovell.yunwei.km_micor_service.util.communicationResumeManage.ReadExcelConferenceMcu;
import com.enovell.yunwei.km_micor_service.util.communicationResumeManage.ReadExcelConferenceOther;
...
@Override
public ResultMsg importConferenceMcu(String publicType, MultipartFile file, String userId, String orgId) {
   	ReadExcelConferenceMcu readExcel =new ReadExcelConferenceMcu();
   	List<ConferenceMcuDto> fdpList = readExcel.getExcelInfo(file);
...
//		    		document.put("source", "1");
		    		...
		    		document.put("portConfQuantity", s.getPortConfQuantity()); 
		    		document.put("portUseQuantity", s.getPortUseQuantity()); 
		    		document.put("portInQuantity", s.getPortInQuantity()); 
...
//	    	 //删除数据库中来源为上一次导入的数据
   ...
   /**
	 ...
	 * @return 
	 */
	public Map<String,Object> checkNullRow(List<ConferenceMcuDto> fdpList) {
	...
	public Map<String,Object> checkWorkshopAndWorkArea(List<ConferenceMcuDto> fdpList) {
		 Map<String, Object> map = new HashMap<String, Object>();
       //获取所有车间
...
@Override
public ResultMsg importConferenceOther(String publicType, MultipartFile file, String userId, String orgId) {
   	ReadExcelConferenceOther readExcel =new ReadExcelConferenceOther();
   	List<ConferenceOtherDto> fdpList = readExcel.getExcelInfo(file);
   	//查询一遍数据库中的所有数据用于和表格中的数据比较
   	List<Document> listAll = this.getAllDocumentByPublicType("deviceRecord",publicType);
   	//存放数据来源于导入的数据的id,用于新导入时删除上一次导入数据
   	List<String> reatList = new ArrayList<String>();
   	...
   	reatList.forEach(System.out::println);
   	//验证表格中的必填数据是否有空
   	Map<String, Object> checkNullMap = checkNullRowOfother(fdpList);
   	Map<String, Object> checkCjMap = checkWorkshopAndWorkAreaOfother(fdpList);
   	if((boolean) checkNullMap.get("isNull")==false){
   		return (ResultMsg) checkNullMap.get("ResultMsg");
   	}
   	//验证填写的车间工区是否正确
...
//	    	 //删除数据库中来源为上一次导入的数据
   ...
   /**
	 ...
	 * @return 
	 */
	public Map<String,Object> checkNullRowOfother(List<ConferenceOtherDto> fdpList) {
	 Map<String, Object> map = new HashMap<String, Object>();
    //检验每一必填列数据中是否有未填，默认为true
  	 boolean workshopNull =fdpList.stream().anyMatch(f->StringUtils.isNotBlank(f.getWorkshop())==false);
  	 boolean workAreaNull =fdpList.stream().anyMatch(f->StringUtils.isNotBlank(f.getWorkArea())==false);
	...
	//验证车间是否为系统中数据，工区是否在车间下
	public Map<String,Object> checkWorkshopAndWorkAreaOfother(List<ConferenceOtherDto> fdpList) {
		 Map<String, Object> map = new HashMap<String, Object>();
       //获取所有车间
		List<String> cjList = this.getCadreAndShop();
	    //workshopCheck为true时，导入的车间全在系统中
   ...
   public Map<String,Object> checkTransSdhDtoNullRow(List<ConferenceOtherDto> fdpList) {
   	Map<String, Object> map = new HashMap<String, Object>();
       //检验每一必填列数据中是否有未填，默认为true
     	 boolean workshopNull =fdpList.stream().anyMatch(f->StringUtils.isNotBlank(f.getWorkshop())==false);
     	 boolean workAreaNull =fdpList.stream().anyMatch(f->StringUtils.isNotBlank(f.getWorkArea())==false);
   ...
   //验证车间是否为系统中数据，工区是否在车间下
 	public Map<String,Object> checkTransSdhDtoWorkshopAndWorkArea(List<ConferenceOtherDto> fdpList) {
 		Map<String, Object> map = new HashMap<String, Object>();
 		//获取所有车间
 		List<String> cjList = this.getCadreAndShop();
 	    //workshopCheck为true时，导入的车间全在系统中
