...
@Service("circuitWorkOrderApproveService")
public class CircuitWorkOrderApproveServiceImpl implements  CircuitWorkOrderApproveService{
	...
	@Override
	public long findAllDocumentCount(String collectionName, String userId, String currentDay, String workOrderName, String flowState, String startUploadDate, String endUploadDate) {
		 try(MongoClient mc = new MongoClient(mongoHost,mongoPort)) {
	            MongoDatabase md = mc.getDatabase(mongoDatabase);
	            Bson filter = getFilter(userId, currentDay, workOrderName, flowState, startUploadDate, endUploadDate);
	            return  md.getCollection(collectionName).count(filter);
	        }
	}

	@Override
	public List<Document> findAllDocument(String collectionName, String userId, String currentDay, String workOrderName, String flowState, String startUploadDate, String endUploadDate, int start, int limit) {
	...
	private void addIssueData(Document doc){
		...
		for(int i=0 ; i<executiveStaffIds.length ; i++){
			...
			document.put("workOrderName", doc.getString("workOrderName"));
			document.put("issueDate", doc.getString("approveTime"));
	...
	private void addDocument(Document doc) {
		  try(MongoClient mc = new MongoClient(mongoHost,mongoPort)){
	            MongoDatabase md = mc.getDatabase(mongoDatabase);
	            doc.append("status",1);
	            md.getCollection("circuitIssueWorkOrder").insertOne(doc);
	        }
	}
	 ...
	 * @return 查询条件 Bson
	 */
    private Bson getFilter(String userId, String currentDay, String workOrderName, String flowState, String startUploadDate, String endUploadDate) {
