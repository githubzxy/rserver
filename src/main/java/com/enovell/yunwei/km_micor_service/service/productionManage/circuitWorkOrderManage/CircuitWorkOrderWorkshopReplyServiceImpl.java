...
@Service("circuitWorkOrderWorkshopReplyService")
public class CircuitWorkOrderWorkshopReplyServiceImpl implements  CircuitWorkOrderWorkshopReplyService{
	...
	@Override
	public void addReplyDocument(Document newDocument,String circuitIssueWorkOrderReply, Document document,String circuitIssueWorkOrder) {
		 try(MongoClient mc = new MongoClient(mongoHost,mongoPort)){
	            MongoDatabase md = mc.getDatabase(mongoDatabase);
	            newDocument.append("status",1);
	            md.getCollection(circuitIssueWorkOrderReply).insertOne(newDocument);
	        }
		 updateDocument(document,circuitIssueWorkOrder,"6.5");
	}
	...
	private void addDistributeDocument(Document doc) {
		  try(MongoClient mc = new MongoClient(mongoHost,mongoPort)){
	            MongoDatabase md = mc.getDatabase(mongoDatabase);
	            doc.append("status",1);
	            md.getCollection("circuitDistributeWorkOrder").insertOne(doc);
	        }
	}
...
//            }
        	Document document = circuitWorkOrderApplyService.findDocumentById(results.get(0).getString("docId"),"circuitWorkOrder");
        	document.put("flowState", "-1");
        	circuitWorkOrderApplyService.updateDocument(document, "circuitWorkOrder");
        }
