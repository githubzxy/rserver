...
import com.enovell.yunwei.km_micor_service.util.communicationResumeManage.ReadExcelDispatchStationChanger;
 ...
 * 类名称：DispatchStationChanger   
 * 类描述: 调度通信系统-车站交换机导入excel实现 
 * 创建人：zhouxingyu
 * 创建时间：2019年6月19日 下午4:28:28
 * 修改人：zhouxingyu 
 * 修改时间：2019年6月19日 下午4:28:28   
 *
 */
@Service("dispatchStationChanger")
public class DispatchStationChanger implements DispatchService{
	...
	@Override
	 public ResultMsg importDispatchTrunk(String publicType, MultipartFile file, String userId, String orgId) {
		 ...
		 * @return 
		 */
		public Map<String,Object> checkNullRow(List<DispatchStationChangerDto> fdpList) {
	     ...
	     //检验每一必填列数据中是否有未填，默认为true
	   	 ...
	   	 boolean userNumberNull =fdpList.stream().anyMatch(f->StringUtils.isNotBlank(f.getUserNumber())==false);
	   	 boolean numOfTrunkConfNull =fdpList.stream().anyMatch(f->StringUtils.isNotBlank(f.getNumOfTrunkConf())==false);
	   	 boolean numOfTrunkUsedNull =fdpList.stream().anyMatch(f->StringUtils.isNotBlank(f.getNumOfTrunkUsed())==false);
	   	 boolean numOfUserConfNull =fdpList.stream().anyMatch(f->StringUtils.isNotBlank(f.getNumOfUserConf())==false);
	   	 boolean numOfUserUsedNull =fdpList.stream().anyMatch(f->StringUtils.isNotBlank(f.getNumOfUserUsed())==false);
	   	 
	   	if(userNumberNull==true) {
	   		map.put("isNull", false);
	   		map.put("ResultMsg", ResultMsg.getFailure("导入失败!请检查'用户数'列是否有空白格"));
	   		return map; 
	   	 }
	   	if(numOfTrunkConfNull==true) {
	   		map.put("isNull", false);
	   		map.put("ResultMsg", ResultMsg.getFailure("导入失败!请检查'2M端口配置容量'列是否有空白格"));
	   		return map; 
	   	 }
	   	if(numOfTrunkUsedNull==true) {
	   		map.put("isNull", false);
	   		map.put("ResultMsg", ResultMsg.getFailure("导入失败!请检查'2M端口实际使用数量'列是否有空白格"));
	   		return map; 
	   	 }
	   	if(numOfUserConfNull==true) {
	   		map.put("isNull", false);
	   		map.put("ResultMsg", ResultMsg.getFailure("导入失败!请检查'值班台数量'列是否有空白格"));
	   		return map; 
	   	 }
		if(numOfUserUsedNull==true) {
	   		map.put("isNull", false);
	   		map.put("ResultMsg", ResultMsg.getFailure("导入失败!请检查'电话分机数量'列是否有空白格"));
	   		return map; 
	   	 }
		...
		public Map<String,Object> checkWorkshopAndWorkArea(List<DispatchStationChangerDto> fdpList) {
			 Map<String, Object> map = new HashMap<String, Object>();
	        //获取所有车间
...
//          Bson filter = getFilter(  workshop,  workArea,  deviceName,  publicType, deviceClass, deviceType, manufacturers);
	...
	public ResultMsg importDispatchStationChanger(String publicType, MultipartFile file, String userId, String orgId) {
    	ReadExcelDispatchStationChanger readExcel =new ReadExcelDispatchStationChanger();
    	List<DispatchStationChangerDto> fdpList = readExcel.getExcelInfo(file);
...
//		    		document.put("source", "1");
		    		...
		    		document.put("userNumber", s.getUserNumber()); 
		    		document.put("numOfTrunkConf", s.getNumOfTrunkConf()); 
		    		document.put("numOfTrunkUsed", s.getNumOfTrunkUsed()); 
		    		document.put("numOfUserConf", s.getNumOfUserConf()); 
		    		document.put("numOfUserUsed", s.getNumOfUserUsed()); 
