...
public class CircuitWorkOrderWorkAreaReplyServiceImpl implements  CircuitWorkOrderWorkAreaReplyService{
	...
	@Override
	public long findDistributeReplyAllDocumentCount(String collectionName, String docId, String distributeOrgId) {
		try(MongoClient mc = new MongoClient(mongoHost,mongoPort)) {
			MongoDatabase md = mc.getDatabase(mongoDatabase);
	        ...
	        if(StringUtils.isNotBlank(distributeOrgId)){
	        	filter = Filters.and(filter,Filters.eq("distributeOrgId",distributeOrgId));
	        }
			return  md.getCollection(collectionName).count(filter);
		}
	}
	...
	@Override
	public List<Document> findDistributeReplyAllDocument(String collectionName, String docId, String distributeOrgId, int start, int limit) {
		List<Document> results = new ArrayList<>();
		try(MongoClient mc = new MongoClient(mongoHost,mongoPort)){
			MongoDatabase md = mc.getDatabase(mongoDatabase);
			Bson filter = Filters.eq("status",1);
	...
	@Override
	public void addReplyDocument(Document newDocument,String circuitDistributeWorkOrderReply, Document document,String circuitDistributeWorkOrder) {
		 try(MongoClient mc = new MongoClient(mongoHost,mongoPort)){
	            MongoDatabase md = mc.getDatabase(mongoDatabase);
	            newDocument.append("status",1);
	            md.getCollection(circuitDistributeWorkOrderReply).insertOne(newDocument);
	        }
		 updateDocument(document,circuitDistributeWorkOrder,"7.5");
	}
