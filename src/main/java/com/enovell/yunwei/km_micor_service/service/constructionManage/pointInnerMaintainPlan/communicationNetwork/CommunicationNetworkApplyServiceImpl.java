...
import com.enovell.yunwei.km_micor_service.dto.CommunicationNetworkApplyDto;
import com.enovell.yunwei.km_micor_service.dto.CommunicationNetworkApplyWordCell;
...
@Service("communicationNetworkApplyService")
public class CommunicationNetworkApplyServiceImpl implements  CommunicationNetworkApplyService{
...
//        filter = Filters.and(filter,Filters.in("summaryPersonId",userId,""));//当前登陆用户ID与汇总人ID相同或者汇总人ID为空
	...
	@Override
	public String exportApplyFrom(CommunicationNetworkApplyDto dto){
...
//		String outFilePath = Day3+"日"+dto.getUnit()+"天窗点外作业项目作业计划申请表"+dto.getSerial()+"号"+".docx";
		...
		try {
			FileInputStream fis = new FileInputStream(new File(pointOuterPath+"communicationNetwork.docx"));
			XWPFDocument xdoc = new XWPFDocument(fis);
			for (XWPFTable table : xdoc.getTables()) {
				if(StringUtils.isNotBlank(dto.getProject())){
					XWPFRun run = table.getRow(CommunicationNetworkApplyWordCell.project.getRow()).getCell(CommunicationNetworkApplyWordCell.project.getCol()).getParagraphArray(0).createRun();
				...
				if(StringUtils.isNotBlank(dto.getType())){
					XWPFRun run = table.getRow(CommunicationNetworkApplyWordCell.type.getRow()).getCell(CommunicationNetworkApplyWordCell.type.getCol()).getParagraphArray(0).createRun();
					run.setFontFamily("宋体");
					run.setFontSize(12);
				...
				if(StringUtils.isNotBlank(dto.getOrgName())){
					XWPFRun run = table.getRow(CommunicationNetworkApplyWordCell.orgName.getRow()).getCell(CommunicationNetworkApplyWordCell.orgName.getCol()).getParagraphArray(0).createRun();
					run.setFontFamily("宋体");
					run.setFontSize(12);
				...
				if(StringUtils.isNotBlank(dto.getCreateDateStr())){
					XWPFRun run = table.getRow(CommunicationNetworkApplyWordCell.createDateStr.getRow()).getCell(CommunicationNetworkApplyWordCell.createDateStr.getCol()).getParagraphArray(0).createRun();
					run.setFontFamily("宋体");
					run.setFontSize(12);
				...
				if(StringUtils.isNotBlank(dto.getCreateUserName())){
					XWPFRun run = table.getRow(CommunicationNetworkApplyWordCell.createUserName.getRow()).getCell(CommunicationNetworkApplyWordCell.createUserName.getCol()).getParagraphArray(0).createRun();
					run.setFontFamily("宋体");
					run.setFontSize(12);
				...
				if(dto.getUploadFileArr().size()!=0){
					...
					XWPFRun run = table.getRow(CommunicationNetworkApplyWordCell.uploadFileArr.getRow()).getCell(CommunicationNetworkApplyWordCell.uploadFileArr.getCol()).getParagraphArray(0).createRun();
					run.setFontFamily("宋体");
					run.setFontSize(12);
					String result = "";
				...
				if(StringUtils.isNotBlank(dto.getSkillAuditor())){
					XWPFRun run = table.getRow(CommunicationNetworkApplyWordCell.skillAuditor.getRow()).getCell(CommunicationNetworkApplyWordCell.skillAuditor.getCol()).getParagraphArray(0).createRun();
					run.setFontFamily("宋体");
					run.setFontSize(12);
				...
				if(StringUtils.isNotBlank(dto.getSkillAuditDate())){
					XWPFRun run = table.getRow(CommunicationNetworkApplyWordCell.skillAuditDate.getRow()).getCell(CommunicationNetworkApplyWordCell.skillAuditDate.getCol()).getParagraphArray(0).createRun();
					run.setFontFamily("宋体");
					run.setFontSize(12);
				...
				if(StringUtils.isNotBlank(dto.getSafeAuditor())){
					XWPFRun run = table.getRow(CommunicationNetworkApplyWordCell.safeAuditor.getRow()).getCell(CommunicationNetworkApplyWordCell.safeAuditor.getCol()).getParagraphArray(0).createRun();
					run.setFontFamily("宋体");
					run.setFontSize(12);
					run.setText(dto.getSafeAuditor());
				}
				if(StringUtils.isNotBlank(dto.getSafeAuditDate())){
					XWPFRun run = table.getRow(CommunicationNetworkApplyWordCell.safeAuditDate.getRow()).getCell(CommunicationNetworkApplyWordCell.safeAuditDate.getCol()).getParagraphArray(0).createRun();
					run.setFontFamily("宋体");
					run.setFontSize(12);
					run.setText(dto.getSafeAuditDate());
				}
				if(StringUtils.isNotBlank(dto.getDispatchAuditor())){
					XWPFRun run = table.getRow(CommunicationNetworkApplyWordCell.dispatchAuditor.getRow()).getCell(CommunicationNetworkApplyWordCell.dispatchAuditor.getCol()).getParagraphArray(0).createRun();
					run.setFontFamily("宋体");
					run.setFontSize(12);
					run.setText(dto.getDispatchAuditor());
				}
				if(StringUtils.isNotBlank(dto.getDispatchAuditDate())){
					XWPFRun run = table.getRow(CommunicationNetworkApplyWordCell.dispatchAuditDate.getRow()).getCell(CommunicationNetworkApplyWordCell.dispatchAuditDate.getCol()).getParagraphArray(0).createRun();
					run.setFontFamily("宋体");
					run.setFontSize(12);
					run.setText(dto.getDispatchAuditDate());
				}
				if(StringUtils.isNotBlank(dto.getApprover())){
					XWPFRun run = table.getRow(CommunicationNetworkApplyWordCell.approver.getRow()).getCell(CommunicationNetworkApplyWordCell.approver.getCol()).getParagraphArray(0).createRun();
					run.setFontFamily("宋体");
					run.setFontSize(12);
					run.setText(dto.getApprover());
				}
				if(StringUtils.isNotBlank(dto.getApproveDate())){
					XWPFRun run = table.getRow(CommunicationNetworkApplyWordCell.approveDate.getRow()).getCell(CommunicationNetworkApplyWordCell.approveDate.getCol()).getParagraphArray(0).createRun();
					run.setFontFamily("宋体");
					run.setFontSize(12);
					run.setText(dto.getApproveDate());
				}
				if(StringUtils.isNotBlank(dto.getSkillAuditAdvice())){
					XWPFRun run = table.getRow(CommunicationNetworkApplyWordCell.skillAuditAdvice.getRow()).getCell(CommunicationNetworkApplyWordCell.skillAuditAdvice.getCol()).getParagraphArray(0).createRun();
					run.setFontFamily("宋体");
					run.setFontSize(12);
					run.setText(dto.getSkillAuditAdvice());
				}
				if(StringUtils.isNotBlank(dto.getSafeAuditAdvice())){
					XWPFRun run = table.getRow(CommunicationNetworkApplyWordCell.safeAuditAdvice.getRow()).getCell(CommunicationNetworkApplyWordCell.safeAuditAdvice.getCol()).getParagraphArray(0).createRun();
					run.setFontFamily("宋体");
					run.setFontSize(12);
					run.setText(dto.getSafeAuditAdvice());
				}
				if(StringUtils.isNotBlank(dto.getDispatchAuditAdvice())){
					XWPFRun run = table.getRow(CommunicationNetworkApplyWordCell.dispatchAuditAdvice.getRow()).getCell(CommunicationNetworkApplyWordCell.dispatchAuditAdvice.getCol()).getParagraphArray(0).createRun();
					run.setFontFamily("宋体");
					run.setFontSize(12);
					run.setText(dto.getDispatchAuditAdvice());
				}
				if(StringUtils.isNotBlank(dto.getApproveAdvice())){
					XWPFRun run = table.getRow(CommunicationNetworkApplyWordCell.approveAdvice.getRow()).getCell(CommunicationNetworkApplyWordCell.approveAdvice.getCol()).getParagraphArray(0).createRun();
					run.setFontFamily("宋体");
					run.setFontSize(12);
