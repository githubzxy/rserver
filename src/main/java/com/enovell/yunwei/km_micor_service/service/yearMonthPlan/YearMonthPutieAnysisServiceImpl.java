...
public class YearMonthPutieAnysisServiceImpl implements YearMonthPutieAnysisService{
	...
	public ResultMsg workShopSummary(String id, User user, String attachPath) {
		...
		//获取车间附件路径
		String summaryFileName = getSumaryFileName(user, attachPath, dto.getYear(), orgNameDatas, reportPaths);
	 ...
	 * @return
	 */
	private String getSumaryFileName(User user, String attachPath, String year,
			List<String> orgNameDatas, List<String> reportPaths) {
		...
		if (YearMonthReportAttachPuTie.ATTACH_PATH8_1.equals(attachPath)) {
			...
			summaryFile = summarySheetData(attachPathValue, user, reportPaths, orgNameDatas);
			...
			summaryFile = tXYearPutieMongoExecuteService.summaryOfTxYear(reportPaths, user, year);
		}
	 ...
	 */
	private String summarySheetData(String workShopFilePath, User user, List<String> workAreaFilePaths,
			List<String> workAreaOrgName) {
		...
		PoiExcelSheetCopy.getExcelwithSheets(reportDatas, summaryFile, 0);
		return summaryFile;
	}
