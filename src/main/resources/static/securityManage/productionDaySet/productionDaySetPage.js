 ...
 */
define('kmms/securityManage/productionDaySet/productionDaySetPage',
	[
	 	'bui/common',
	 	'bui/data',
	 	'bui/grid',
    ...
    var AccidentTroubleObstacleQueryPage = BUI.Component.Controller.extend(
        {
            initializer : function(){
               ...
               // _self.addChild(_self._initSearchGridContainer());
                _self.addChild(_self._initContent());
            },
            renderUI:function(){
                var _self=this;
               // _self._initDate();
...
//                _self._initOrganizationPicker();
            ...
            bindUI:function(){
                ...
                $('#updateNum').on('click',function(e){
					var _self = this;
                	var dayNum = $("#dayNum").val();
                    $.ajax({
                        url:'/kmms/dayNumAction/updateNum',
                        type:'post',
                        data:{collectionName:"dayNum",dayNum:dayNum},
                        dataType:"json",
                        success:function(res){
                        	BUI.Message.Confirm('设置成功', function() {
...
//                var orgPicker=_self.get('orgPicker');
            ...
            _initNum:function(){
            	var _self = this;
                $.ajax({
                    url:'/kmms/dayNumAction/findNum',
                    type:'post',
                    data:{collectionName:"dayNum"},
                    dataType:"json",
                    success:function(res){
                   		 $("#dayNum").val(res.data.dayNum);

                    }
                })
            },
            _initContent:function(){
        			var title = new BUI.Component.Controller({
        				height:400,
        				content : "<div style='text-align:center;margin-top: 200px;'><div style='display:inline;margin-right:30px;font-size:20px;'>安全生产天数基准值:</div>" +
        						"<div style='margin-right:50px;display:inline;' class='controls'><input type='text' name='dayNum' id='dayNum' style='width: 100px;height:26px;'></input> <span>天</span></div>" +
        						"<div style='display:inline' ><button class='button  button-primary' id='updateNum' style='width:60;height:30px;' >保存</button></div></div></div>",
        				/*elStyle : {class="button button-primary"
        					'background': '#F7F7F7',
        					'margin': 'auto',
