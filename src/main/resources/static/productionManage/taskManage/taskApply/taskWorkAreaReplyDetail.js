 ...
 * @date 19-5-13
 */
define('kmms/productionManage/taskManage/taskApply/taskWorkAreaReplyDetail',
		[
		 	'bui/common',
		 	'bui/form',
		 	'bui/grid',
			...
			'kmms/productionManage/taskManage/taskApply/taskWorkAreaDistributeReplyDetail'
		],function(r){
    var BUI = r('bui/common'),Select = r('bui/select'),Data = r('bui/data'),
    	  ...
    	  taskWorkAreaDistributeReplyDetail = r('kmms/productionManage/taskManage/taskApply/taskWorkAreaDistributeReplyDetail');
    var taskWorkAreaReplyDetail = BUI.Overlay.Dialog.extend({
        ...
        renderUI:function(){
			var _self=this;
			$("#taskWorkAreaReplyDetail .bui-ext-close").css("display","none");
			$('.taskWorkAreaReplyDetailDialogClass .bui-stdmod-body').attr("style","overflow-y:hidden;overflow-x:hidden;");
			$('.taskWorkAreaReplyDetailDialogClass .bui-grid .bui-grid-body').attr("style","overflow-y:scroll;overflow-x:auto;height:300px;");
...
//                console.log(record);
                ...
                if(target.hasClass('replyDetailBtn')){
                    var infoDialog = new taskWorkAreaDistributeReplyDetail({
                        shiftId:docId,
                        distributeOrgId:distributeOrgId
                    });
                    Mask.maskElement('#taskWorkAreaReplyDetail');
                    infoDialog.show();
                    infoDialog.on('closeDialog',function(e){
                    	infoDialog.close();
	                	Mask.unmaskElement('#taskWorkAreaReplyDetail');
	                });
                }
            });
...
//                        }
		...
		_initGridData : function(){
			...
			var store = new Data.Store({
				url:'/kmms/taskWorkAreaReplyAction/findAll',
				pageSize:10,
				params:{docId:_self.get('shiftId'),executiveOrgId:_self.get('executiveOrgId'),collectionName : "IssueTaskOfWorkArea"},
				proxy:{
					method:'post',
					dataType:'json'
				},
      ...
      /*  _getShowData : function(){
        ...
        ATTRS : {
            id:{value : 'taskWorkAreaReplyDetailDialog'},
            elAttrs:{value: {id:"taskWorkAreaReplyDetail"}},
            elCls : {value:'taskWorkAreaReplyDetailDialogClass'},
            title:{value:'工区回复详情'},
            width:{value:800},
            height:{value:400},
    ...
    return taskWorkAreaReplyDetail;
});