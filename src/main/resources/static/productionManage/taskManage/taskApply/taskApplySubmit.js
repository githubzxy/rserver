 ...
 * @date 19-5-13
 */
define('kmms/productionManage/taskManage/taskApply/taskApplySubmit',
		[
		 	'bui/common',
		 	'bui/form',
		 	'bui/select',
    ...
    var taskApplySubmit = BUI.Overlay.Dialog.extend({
...
//                elStyle:{overflowY:'scroll',height:'400px'}
        ...
        ATTRS : {
            id:{value : 'taskApplySubmitDialog'},
            elAttrs:{value: {id:"taskApplySubmit"}},
            title:{value:'提交'},
            width:{value:650},
            height:{value:400},
            ...
            submit:{
                value : function(){
                    var _self = this;
                    var formAdd = _self.getChild('taskApplySubmitForm');
                    //获取上传文件
	        		...
	        		if(uploadFileArr.length==0){
	        			Mask.maskElement('#taskApplySubmit');
	        			BUI.Message.Alert("请上传附件！",function(){
	        				Mask.unmaskElement('#taskApplySubmit');
	        			},'error');
	        			$("#taskApplySubmit .bui-message .bui-ext-close").css("display","none");
	        			return;
	        		}
	        		...
	        		var pl = new PostLoad({
        				url : '/zuul/kmms/taskApplyAction/submitDoc',
        				el : _self.get('el'),
        				loadMsg : '上传中...'
        			}); 
...
//	        		data.flowState="0";
	        		data.id=_self.get('shiftId');
	        		var pl = new PostLoad({
        				url : '/zuul/kmms/taskApplyAction/updateDoc',
        				el : _self.get('el'),
        				loadMsg : '上传中...'
        			}); 
        			pl.load(data, function(e){
