                ...
                table.on('cellclick',function(ev){
                    ...
                    if(target.hasClass('replyBtn')){
                        var approveDialog = new taskWorkAreaReplyAdd({
                            ...
                            taskWorkAreaReply:_self.get('taskWorkAreaReply'),
                        	IssueTaskOfWorkArea:_self.get('IssueTaskOfWorkArea'),
                    ...
                    if(target.hasClass('finishBtn')){
                    	BUI.Message.Confirm('确认要完成吗？完成后则不能继续回复，请确认是否任务已完成！',function(){
 	                       var postLoad = new PostLoad({
 	                           url : '/kmms/taskWorkAreaReplyAction/finishDoc.cn',
 	                           el : _self.get('el'),
 	                           loadMsg : '执行中...'
 	                       });
 	                       postLoad.load({id:docId,distributeOrgId:_self.get("orgId"),flowState:"8",collectionName:_self.get('IssueTaskOfWorkArea')},function (res) {
 	                    	   tbar.msg(res);
 	                    	   store.load();
 	                       });
                 	   },'question');
...
//                    permissionStore : _self._initPermissionStore()
        ...
        },
        {
            ATTRS : {
                ...
                taskWorkAreaReply : {value:'taskWorkAreaReply'},//存储表名（派发子单的回复子单：存放车间直接回复的内容）
