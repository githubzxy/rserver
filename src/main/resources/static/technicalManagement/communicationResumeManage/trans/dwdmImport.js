...
define('kmms/technicalManagement/communicationResumeManage/trans/dwdmImport',[
	...
	var dwdmImport = Overlay.Dialog.extend({
		...
		bindUI : function(){
			var _self = this;
			var publicType = _self.get('publicType');
			/**
			 * 下载模板方法
			 */
			$('#downLoadTemplate').on('click',function(){
				window.location.href = '/kmms/exportModel/'+publicType+'.xls';
			});
			//定义按键
			var buttons = [
				{
			...
			success : {
				value : function(){
					...
					if(!importForm.isValid()){
	                    ...
	                    formData.append('publicType',_self.get('publicType'));
	                    var xhr = new XMLHttpRequest();
	                    xhr.open("POST", "/kmms/importAction/Import");
	                    xhr.send(formData);
