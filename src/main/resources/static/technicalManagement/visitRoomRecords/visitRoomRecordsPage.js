 ...
 * @date 19-3-27
 */
define('kmms/technicalManagement/visitRoomRecords/visitRoomRecordsPage',['bui/common',
    'bui/data','bui/grid','bui/calendar',
    'common/org/OrganizationPicker',
    'common/grid/SearchGridContainer',
    'common/data/PostLoad',
    'bui/select',
    'kmms/technicalManagement/visitRoomRecords/visitRoomRecordsAdd',
    'kmms/technicalManagement/visitRoomRecords/visitRoomRecordsDetail',
    'kmms/technicalManagement/visitRoomRecords/visitRoomRecordsUpdate',
    'kmms/technicalManagement/visitRoomRecords/SelectSuggest'],function(require){
    var BUI = require('bui/common');
    var Grid = require('bui/grid');
    var Data = require('bui/data');
    var Calendar = require('bui/calendar');
    var Select = require('bui/select');
    var SelectSuggest = require("kmms/technicalManagement/visitRoomRecords/SelectSuggest");
    ...
    var visitRoomRecordsPageNewAdd = require('kmms/technicalManagement/visitRoomRecords/visitRoomRecordsAdd');
    var visitRoomRecordsPageNewUpdate = require('kmms/technicalManagement/visitRoomRecords/visitRoomRecordsUpdate');
    var visitRoomRecordsPageNewDetails = require('kmms/technicalManagement/visitRoomRecords/visitRoomRecordsDetail');
    var SEARCH_FORM_ID = 'searchForm';
    var visitRoomRecordsPage = BUI.Component.Controller.extend(
        {
...
//    			_self._initRailAndRoom();
            ...
            bindUI:function(){
                ...
                $('.addBtn').on('click',function(e){
                    var addDialog = new visitRoomRecordsPageNewAdd({
                ...
                table.on('cellclick',function(ev){
                    ...
                    if(target.hasClass('infoBtn')){
                    	var infoDialog = new visitRoomRecordsPageNewDetails({
                            collectionName:_self.get('collectionName'),
                    		userId:_self.get('userId'),
                    		shiftId:docId
                        });
                    ...
                    if(target.hasClass('editBtn')){
                        var editDialog = new visitRoomRecordsPageNewUpdate({
                            collectionName:_self.get('collectionName'),
                            userId:_self.get('userId'),
                            shiftId:docId
                        });
                	...
                	items: [
                        {
                            ...
                            label : '接待人',
                            item : '<input type="text" name="helper" id="helper" style="width: 175px;" class="input-normal"/>'
                        },{
                            label : '开始时间',
...
//                    },
                    {
                        ...
                        width:"20%",
                        renderer:function (e) {
                        	return '<span  class="grid-command editBtn">修改</span>'+'<span  class="grid-command infoBtn">详情</span>';
                        }
                    }
                ];
                return columns;
            ...
            _initStore:function () {
                ...
                var store = new Data.Store({
                    url : "/kmms/visitRoomRecordsAction/findAllDatas",
                    autoLoad : true ,
                    pageSize : 10,
                    proxy : {
                        method : 'post',
		...
		_exportXls:function(records){
			...
			for(var i = 0 ; i < records.length ; i++){
				...
				json += '{'
					...
					+'"visitDate":'+'"'+Grid.Format.datetimeRenderer(row.visitDate)+'",'
					+'"visitName":'+'"'+row.visitName+'",'
					+'"visitFrom":'+'"'+row.visitFrom+'",'
					+'"helper":'+'"'+row.helper+'",'
					+'"jobContent":'+'"'+row.jobContent+'",'
					+'"leaveDate":'+'"'+Grid.Format.datetimeRenderer(row.leaveDate)+'",'
					+'"remark":'+'"'+Grid.Format.datetimeRenderer(row.remark)+'"'
					+'},';
			}
                    ...
                    tbarItems : [
                        {
                            ...
                            +'<form action="/kmms/visitRoomRecordsAction/exportXls" id="exportForm" method="post">'
        ...
        {
            ATTRS : {
            	elCls : {value:'visitRoomRecordsInfoPageCls'},
            	perId : {},
                userId : {},//登录用户ID
                userName : {},//登录用户名称
                orgId : {},//登录用户组织机构ID
                 ...
                 * 存储表名(机房台账表名)
                 */
                collectionName : {value:'visitRoomRecordsInfo'}
            }
        });
    return visitRoomRecordsPage;
});