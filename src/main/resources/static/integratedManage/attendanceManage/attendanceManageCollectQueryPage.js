...
define('kmms/integratedManage/attendanceManage/attendanceManageCollectQueryPage',
	[
	 	...
	 	'kmms/integratedManage/attendanceManage/attendanceManageCollectSum',
	 	'kmms/integratedManage/attendanceManage/attendanceManageCollectRangeSum',
	],function (r) {
    var BUI = r('bui/common'),
    	...
    	attendanceManageCollectRangeSum = r('kmms/integratedManage/attendanceManage/attendanceManageCollectRangeSum'),
    	attendanceManageCollectSum = r('kmms/integratedManage/attendanceManage/attendanceManageCollectSum'),
    ...
    var AttendanceManageCollectQueryPage = BUI.Component.Controller.extend(
        {
            ...
            bindUI:function(){
                ...
                $('.addBtn').on('click',function(e){
                    var addDialog = new attendanceManageCollectSum({
                    ...
                    addDialog.on('completeAddSave',function(e){
                        tbar.msg(e.result);
                        window.location.href = "/kmms/commonAction/download?path=" + e.result.data+'&fileName='+"中国铁路昆明局集团有限公司"+e.yearmonth+"考勤统计表.xlsx";// 下载路径
                        addDialog.close();
                        store.load();
                    });
                });
                ...
                $('.addRangeBtn').on('click',function(e){
                	var addDialog = new attendanceManageCollectRangeSum({
                		attendanceManage:_self.get('attendanceManage'),
                		attendanceManageCollect:_self.get('attendanceManageCollect'),
                		userId:_self.get('userId'),
                		userName:_self.get('userName'),
...
//                		console.log(e.result);
                ...
                table.on('cellclick',function(ev){
                    var record = ev.record, //点击行的记录
                    	  ...
                    	  attendanceOrgId = record.attendanceOrgId;
                    ...
                    if(target.hasClass('submitBtn')){
                    	...
                    	BUI.Message.Confirm('确认审核通过吗？',function(){
 	                       var postLoad = new PostLoad({
 	                           url : '/kmms/attendanceManageCollectQueryAction/updateCollectDoc.cn',
 	                           el : _self.get('el'),
 	                           loadMsg : '提交中...'
 	                       });
                    	   postLoad.load({id:docId,flowState:"-1",workshopQueryData:work,filePath:filePath,date:date,attendanceOrgName:attendanceOrgName,attendanceOrgId:attendanceOrgId,attendanceManageCollect:_self.get('attendanceManageCollect')},function (res) {
                    		   tbar.msg(res);
                    		   store.load();
                    	   });
                 	   },'question');
                ...
                var columns = [
                	{
                        ...
                        renderer:function (value) {
                        	...
                        	if(value=='1'){
                        		...
                        		button += '<span  class="grid-command submitBtn">通过</span>';
                        	...
                        	if(value=='-1'){
                        		...
                        		button += '<span  class="grid-command submitBtn">汇总</span>';
                        		button += '<span  class="grid-command backBtn">退回</span>';
                        		button += '<span  class="grid-command searchBtn">查看</span>';
                        		button += '<span  class="grid-command downBtn">下载</span>';
                        		button += '<span  class="grid-command importBtn">导入</span>';
...
//                            text : '<i class="icon-download"></i>导出单月统计表',
//                        },
                        {
                            id : 'add',
                            btnCls : 'button button-small addRangeBtn',
                            text : '<i class="icon-download"></i>按月份范围导出统计表',
                        },
//                        {
