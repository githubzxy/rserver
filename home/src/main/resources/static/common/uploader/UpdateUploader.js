 ...
 */
seajs.use('common/uploader/uploader.css');
define('common/uploader/UpdateUploader', ['bui/common','bui/uploader', 'bui/progressbar'], function (r) {
    var BUI = r('bui/common'),Uploader = r('bui/uploader'), Theme = Uploader.Theme, ProgressBar = r('bui/progressbar'),
    ...
    Theme.addTheme('updateTheme', {//自定义附件组件的显示主题
        elCls: 'defaultTheme',
        queue: {
        	itemTpl: '<li>{resultTpl} <span class="action">'+
        		'<a class="bui-queen-item-download" href="/kmms/atachFile/download?name={name}&path={path}">下载</a>'+
        		'<a class="bui-queen-item-edit" href="/pageoffice/?filePath={path}"  target="_blank">编辑</a>'+
        		'<span class="' + CLS_QUEUE_ITEM + '-del">删除</span>'+
        		'</span></li>'
        	,
            //结果的模板，可以根据不同状态进进行设置
            resultTpl: {
                'default': '<div class="default">{name}</div>',
                'default': '<div class="error"><span title="{name}"></span><span class="uploader-error">文件名不能存在..特殊字符</span></div>',
    ...
    var UpdateUploader = Uploader.Uploader.extend({
        ...
        bindUI: function () {
            ...
            _self.on('change', function (e) {
                var itemArr = e.items, fileSliceSize = 2097152*125;
				itemArr.forEach(function (item) {
					var name = item.name;
					if (name.indexOf("..") != -1) {
						queue.updateFileStatus(item, 'default');
					}
//					_self.upload();
				});
...
//                           url:'/zuul/std/atachFile/checkFile' ,
                           url:'/zuul/kmms/atachFile/checkFile' ,
...
//            xhr.open("POST", "/zuul/std/atachFile/uploadSlice");
            xhr.open("POST", "/zuul/kmms/atachFile/uploadSlice");
            xhr.send(formData);
...
//                    ext: ['.doc,.docx,.xls,.xlsx', '文件类型只能为{0}'],
                    //文件大小的最大值,单位也是kb
