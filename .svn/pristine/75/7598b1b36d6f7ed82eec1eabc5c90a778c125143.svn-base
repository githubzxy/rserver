...
import com.enovell.yunwei.km_micor_service.util.communicationResumeManage.ReadExcelDispatchDuty;
 ...
 * 类名称：Duty   
 * 类描述: 调度通信系统-车站值班台导入excel实现 
 * 创建人：zhouxingyu
 * 创建时间：2019年6月19日 下午4:29:16
 * 修改人：zhouxingyu 
 * 修改时间：2019年6月19日 下午4:29:16   
 *
 */
@Service("duty")
public class Duty implements DispatchService{
	...
	@Override
	 public ResultMsg importDispatchTrunk(String publicType, MultipartFile file, String userId, String orgId) {
		 ...
		 * @return 
		 */
		public Map<String,Object> checkNullRow(List<DispatchDutyDto> fdpList) {
	     ...
	     //检验每一必填列数据中是否有未填，默认为true
	   	 ...
	   	 boolean numbeDigitalNull =fdpList.stream().anyMatch(f->StringUtils.isNotBlank(f.getNumbeDigital())==false);
	   	 boolean numberDispatchNull =fdpList.stream().anyMatch(f->StringUtils.isNotBlank(f.getNumberDispatch())==false);
	   	 
	   	 
	   	if(numberDispatchNull==true) {
	   		map.put("isNull", false);
	   		map.put("ResultMsg", ResultMsg.getFailure("导入失败!请检查'所属调度操作台序号'列是否有空白格"));
	   		return map; 
	   	 }
	   	if(numbeDigitalNull==true) {
	   		map.put("isNull", false);
	   		map.put("ResultMsg", ResultMsg.getFailure("导入失败!请检查'所属数字环序号'列是否有空白格"));
	   		return map; 
	   	 }
		...
		public Map<String,Object> checkWorkshopAndWorkArea(List<DispatchDutyDto> fdpList) {
			 Map<String, Object> map = new HashMap<String, Object>();
	        //获取所有车间
...
//          Bson filter = getFilter(  workshop,  workArea,  deviceName,  publicType, deviceClass, deviceType, manufacturers);
	...
	public ResultMsg importDispatchDuty(String publicType, MultipartFile file, String userId, String orgId) {
		ReadExcelDispatchDuty readExcel = new ReadExcelDispatchDuty();
    	List<DispatchDutyDto> fdpList = readExcel.getExcelInfo(file);
...
//		    		document.put("source", "1");
		    		...
		    		document.put("numbeDigital", s.getNumbeDigital()); 
		    		document.put("numberDispatch", s.getNumberDispatch()); 
