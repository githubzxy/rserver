 ...
 * @date 19-4-1
 */
define('kmms/productionManage/JobRecord/jobRecordPage', ['bui/common', 'bui/data', 'bui/grid',
    ...
    var jobRecordPage = BUI.Component.Controller.extend({
        ...
        renderUI: function() {
            ...
            _self._initDate();
            _self._initProject();
            if (_self.get("orgType") == 1502) {//1502==车间类别
                $("#workarea").val(_self.get("orgName"));
                $("#workareaId").val(_self.get("orgId"));
                _self._initOrganizationPicker1502();
            }
            if (_self.get("orgType") == 1501 || _self.get("orgType") == 1500) {//1501=科室类别；1500=段
        ...
        bindUI: function() {
            ...
            if (_self.get("orgType") == 1501 || _self.get("orgType") == 1500) {//科室-1501；段-1500
            ...
            table.on('cellclick', function(ev) {
                ...
                if (target.hasClass('infoBtn')) {
                    var infoDialog = new jobRecordDetail({
                        collectionName: _self.get('collectionName'),
                        userId: _self.get('userId'),
                        shiftId: docId
                    });
        ...
        _exportXls: function(records) {
        	 ...
        	 var projectDiv = $('input[name="project"]').val();
        	 if (_self.get("orgType") == 1503){//工区页面能够导入并且只能看到本工区导入的数据;1503==工区类别
        		  workareaId = _self.get("orgId");
        	 }else if(_self.get("orgType") == 1502){//车间页面不能导入只能查看本车间管辖的工区导入的数据；1502==车间类别
        		 workareaId = $('#workareaId').val();
        	 }else{//领导页面不能导入能查看所有工区导入的数据
        		 workshopId = $('#workshopId').val();
        	 }
        	$('#workareaIdOfexport').val(workareaId);
 			$('#workshopIdOfexport').val(workshopId);
 			$('#startDateOfexport').val(startDate);
 			$('#endDateOfexport').val(endDate);
 			$('#projectDivOfexport').val(projectDiv);
        ...
        _initColumns: function() {
            ...
            var columns = [{
                ...
                renderer: function(e) {
                    ...
                    if (_self.get("orgType") == 1503) { //1503==工区类别
                    ...
                    } else if (_self.get("orgType") == 1502) { //1502==车间类别
        ...
        _initStore: function() {
            ...
            var store = new Data.Store({
                url: "/kmms/jobRecordAction/findAll.cn",
                autoLoad: true,
                pageSize: 20,
                proxy: {
                    method: 'post',
        ...
        _initSearchGrid: function() {
            var _self = this;
            var searchGrid = null;
            if (_self.get("orgType") == 1503 || _self.get("orgType") == 1502) { //车间1502和工区1503
                searchGrid = {
                    tbarItems: [{
                        id: 'import',
                        ...
                        text: '<i class="icon-download "></i>导出' +
                            ...
                            '<input type="hidden" name="workareaIdOfexport" id="workareaIdOfexport" />' +
                            '<input type="hidden" name="workshopIdOfexport" id="workshopIdOfexport" />' +
                            '<input type="hidden" name="startDateOfexport" id="startDateOfexport" />' +
                            '<input type="hidden" name="endDateOfexport" id="endDateOfexport" />' +
                            '<input type="hidden" name="projectDivOfexport" id="projectDivOfexport" />' +
                            '</form>',
                    }],
                    plugins: [
