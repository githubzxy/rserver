 ...
 */
define("kmms/productionManage/JobRecord/SelectSuggest",
		[
			"bui/common",
			'bui/select',
		],function(r){
	...
	var SelectSuggest = BUI.Component.Controller.extend({
		...
		resetRender:function(renderData){
			var _self = this;
			var suggest = _self.get("suggestSelect");
			suggest.set("data",renderData);
			suggest.render();
		},
		_initSelectSuggest:function(){
			...
			_self.set("suggestSelect",suggest);
			suggest.render();
			suggest.get('picker').set('elCls', 'suggestApplication');
			$(".suggestApplication").attr("style","height:300px;width:200px;overflow-y:auto;overflow-x:auto;");//border:1px solid #C3C3D6;border-radius:4px
		},
	},{ATTRS:{
