...
import com.enovell.yunwei.km_micor_service.util.communicationResumeManage.ReadExcelDispatchOther;
 ...
 * 类名称：DispatchOther   
 * 类描述:  调度通信系统-其它设备excel导入实现
 * 创建人：zhouxingyu
 * 创建时间：2019年6月19日 下午4:26:49
 * 修改人：zhouxingyu 
 * 修改时间：2019年6月19日 下午4:26:49   
 *
 */
@Service("dispatchOther")
public class DispatchOther implements DispatchService{
	...
	@Override
	 public ResultMsg importDispatchTrunk(String publicType, MultipartFile file, String userId, String orgId) {
		 ...
		 * @return 
		 */
		public Map<String,Object> checkNullRow(List<DispatchOtherDto> fdpList) {
	     ...
	     //检验每一必填列数据中是否有未填，默认为true
	   	 ...
	   	 boolean capacityNull =fdpList.stream().anyMatch(f->StringUtils.isNotBlank(f.getCapacity())==false);
	   	 
	   	if(capacityNull==true) {
	   		map.put("isNull", false);
	   		map.put("ResultMsg", ResultMsg.getFailure("导入失败!请检查'容量'列是否有空白格"));
	   		return map; 
	   	 }
		...
		public Map<String,Object> checkWorkshopAndWorkArea(List<DispatchOtherDto> fdpList) {
			 Map<String, Object> map = new HashMap<String, Object>();
	        //获取所有车间
...
//          Bson filter = getFilter(  workshop,  workArea,  deviceName,  publicType, deviceClass, deviceType, manufacturers);
	...
	public ResultMsg importDispatchOther(String publicType, MultipartFile file, String userId, String orgId) {
		ReadExcelDispatchOther readExcel =new ReadExcelDispatchOther();
    	List<DispatchOtherDto> fdpList = readExcel.getExcelInfo(file);
