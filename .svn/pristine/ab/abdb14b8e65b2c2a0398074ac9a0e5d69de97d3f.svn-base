 ...
 * @date 19-6-19
 */
define('kmms/technicalManagement/communicationResumeManage/conference/mcuEdit',['bui/common',
	'common/form/FormContainer',
    'bui/form',
    ],function(r){
    var BUI = r('bui/common'),
        FormContainer= r('common/form/FormContainer');
    var mcuEdit = BUI.Overlay.Dialog.extend({
...
//        },
            ...
            $.ajax({
            	...
            	/*	    $('#formContainer #deviceId').val(data.deviceId); */
            		   ...
            		   /* $('#formContainer #totalCapacity').val(data.totalCapacity);
            		    ...
            		    $('#formContainer #portConfQuantity').val(data.portConfQuantity);
            		    $('#formContainer #portUseQuantity').val(data.portUseQuantity);
            		    $('#formContainer #portInQuantity').val(data.portInQuantity);
            ...
            var childs = [
            	{
                    ...
                    redStarFlag : true,
                    itemColspan : 1,
                    item : '<input type="text" name="capacity" id="capacity" style="width:99%" data-rules="{required:true}"/>'
                },{
                    label : '设备厂家：',
                    redStarFlag : true,
                    itemColspan : 1,
                    ...
                    label : '总容量（波道）：',
                    itemColspan : 1,
                    item : '<input type="text" name="totalCapacity" id="totalCapacity" style="width:98.5%" />'
                },{
                    label : '路产容量（波道）：',
                    itemColspan : 1,
                    ...
                    redStarFlag : true,
                    itemColspan : 1,
                    item : '<input type="text" name="configChannel" id="configChannel" style="width:98.5%" data-rules="{required:true}"/>'
                },{
                    label : '资产比例：',
                    itemColspan : 1,
                    ...
                    redStarFlag : true,
                    itemColspan : 1,
                    item : '<input type="text" name="portConfQuantity" id="deviceOperationStatus" style="width:98.5%" data-rules="{required:true}"/>'
                },{
                    label : 'MCU端口使用数量：',
                    redStarFlag : true,
                    itemColspan : 1,
                    item : '<input type="text" name="portUseQuantity" id="deviceOperationStatus" style="width:98.5%" data-rules="{required:true}"/>'
                },{
                    label : '接入会议终端数量：',
                    redStarFlag : true,
                    itemColspan : 1,
                    item : '<input type="text" name="portInQuantity" id="deviceOperationStatus" style="width:98.5%" data-rules="{required:true}"/>'
                },{
                    label : '固资编号：',
                    itemColspan : 2,
        ...
        ATTRS : {
            id : {value : 'mcuEditDialog'},
            elAttrs : {value: {id:"mcuPointEdit"}},
            title:{value:'MCU-01'},
            width:{value:630},
            height:{value:500},
            contextPath : {},
            ...
            success:{
                value : function(){
                    var _self = this;
                    var formAdd = _self.getChild('mcuEditForm'),delData=_self.get('delData');
                    //验证不通过
	        		if(!formAdd.isValid()){
	        			return;
	        		}
                    ...
                    xhr.open("POST", "/zuul/kmms/conferenceAction/mcu/updateDoc");
                    xhr.send(formData);
