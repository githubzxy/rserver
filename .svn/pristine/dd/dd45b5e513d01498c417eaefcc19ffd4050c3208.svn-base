...
@Service("communicationNetworkDispatchAuditService")
public class CommunicationNetworkDispatchAuditServiceImpl implements  CommunicationNetworkDispatchAuditService{
...
//        filter = Filters.and(filter,Filters.eq("parentId",orgId));//表示查询的是当前登陆用户所属组织机构下的所有机构的数据
        if(orgId.equals("402891b45b5fd02c015b74c97d740037")){//调度组织id
        	 filter = Filters.and(filter,Filters.in("flowState","2","4"));//表示可以查询技术科审核通过后的数据
            }else{
            filter = Filters.and(filter,Filters.in("flowState","无"));//表示不查询数据
            }
        return filter;
    }
