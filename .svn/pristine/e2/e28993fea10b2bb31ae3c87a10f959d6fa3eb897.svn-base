 ...
 * @date 19-5-13
 */
define('kmms/productionManage/taskManage/taskApply/taskApplyCheckEdit',
		[
		 	'bui/common',
		 	'bui/form',
		 	'bui/select',
    ...
    var taskApplyCheckEdit = BUI.Overlay.Dialog.extend({
        ...
        ATTRS : {
            id:{value : 'taskApplyCheckEditDialog'},
            elAttrs:{value: {id:"taskApplyCheckEdit"}},
            title:{value:'重发'},
            width:{value:650},
            height:{value:420},
            ...
            apply:{
                value : function(){
                    ...
                    var formAdd = _self.getChild('taskApplyCheckEditForm');
                    //获取上传文件
	        		...
	        		if(uploadFileArr.length==0){
	        			Mask.maskElement('#taskApplyCheckEdit');
	        			BUI.Message.Alert("请上传附件！",function(){
	        				Mask.unmaskElement('#taskApplyCheckEdit');
	        			},'error');
	    	    	...
	    	    	if(checkedStatus){
	    	    		if(!data.receivePeople){
		        			...
		        			$("#taskApplyCheckEdit .bui-message .bui-ext-close").css("display","none");
		        			return;
		        		}
	    	    	}
