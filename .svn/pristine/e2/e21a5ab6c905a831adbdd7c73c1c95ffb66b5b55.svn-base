...
//            }
	...
	@Override
	public void addReplyDocument(Document newDocument,String taskWorkAreaReply, Document document,String IssueTaskOfWorkArea) {
		 try(MongoClient mc = new MongoClient(mongoHost,mongoPort)){
	            MongoDatabase md = mc.getDatabase(mongoDatabase);
	            newDocument.append("status",1);
	            md.getCollection(taskWorkAreaReply).insertOne(newDocument);
	        }
		 updateDocument(document,IssueTaskOfWorkArea,"7.5");
	}
