...
import com.enovell.yunwei.km_micor_service.util.communicationResumeManage.ReadExcelWiredWired;
...
public class WiredServiceImpl implements WiredService{
 ...
 */
	...
	@Override
	 public ResultMsg importWiredWired(String publicType, MultipartFile file, String userId, String orgId) {
	    	ReadExcelWiredWired readExcel =new ReadExcelWiredWired();
	    	List<WiredWiredDto> fdpList = readExcel.getExcelInfo(file);
...
//		    	 //删除数据库中来源为上一次导入的数据
		 ...
		 * @return 
		 */
		public Map<String,Object> checkNullRow(List<WiredWiredDto> fdpList) {
		...
		public Map<String,Object> checkWorkshopAndWorkArea(List<WiredWiredDto> fdpList) {
			 Map<String, Object> map = new HashMap<String, Object>();
	        //获取所有车间
	...
	/*
	 * @Override public ResultMsg importWiredWired(String publicType, MultipartFile
	 * file, String userId, String orgId) { ReadExcelTransSdh readExcel =new
	 * ReadExcelTransSdh(); List<TransSdhDto> fdpList =
	 * readExcel.getExcelInfo(file); //查询一遍数据库中的所有数据用于和表格中的数据比较 List<Document>
	 * listAll = this.getAllDocumentByPublicType("deviceRecord",publicType);
	 * //存放数据来源于导入的数据的id,用于新导入时删除上一次导入数据 List<String> reatList = new
	 * ArrayList<String>(); //表格中的数据和数据库中的数据做比较，将数据库中相同的数据中存放在新list中
	 * listAll.stream().forEach(s->{ // if(StringUtils.isNotBlank((String)
	 * s.get("source"))==false){ reatList.add((String)s.get("docId")); } });
	 * //删除数据库中来源为上一次导入的数据 this.removeDocument(reatList, "deviceRecord");
	 * //验证表格中的必填数据是否有空 Map<String, Object> checkNullMap =
	 * checkTransSdhDtoNullRow(fdpList); Map<String, Object> checkCjMap =
	 * checkTransSdhDtoWorkshopAndWorkArea(fdpList); if((boolean)
	 * checkNullMap.get("isNull")==false){ return (ResultMsg)
	 * checkNullMap.get("ResultMsg"); } //验证填写的车间工区是否正确 else if((boolean)
	 * checkCjMap.get("isExist")==false) { return (ResultMsg)
	 * checkCjMap.get("ResultMsg"); }else { fdpList.stream().forEach(s->{ String
	 * collectionName ="deviceRecord"; Document document = new Document();
	 * SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss"); String
	 * creatDateStr = ""; try { creatDateStr = sdf.format(new Date()); } catch
	 * (Exception e) { e.printStackTrace(); } document.put("creatDateStr",
	 * creatDateStr); document.put("publicType", publicType); //
	 * document.put("source", "1");
	 * 
	 * document.put("workshop", s.getWorkshop()); document.put("workArea",
	 * s.getWorkArea()); document.put("combinationClass", s.getCombinationClass());
	 * document.put("deviceClass", s.getDeviceClass()); document.put("deviceCode",
	 * s.getDeviceCode()); document.put("deviceName", s.getDeviceName());
	 * document.put("deviceId", s.getDeviceId()); document.put("systemName",
	 * s.getSystemName()); document.put("site_station_line",
	 * s.getSite_station_line()); document.put("site_station_name",
	 * s.getSite_station_name()); document.put("site_station_place",
	 * s.getSite_station_place()); document.put("site_range_line",
	 * s.getSite_range_line()); document.put("site_range_post",
	 * s.getSite_range_post()); document.put("site_range_place",
	 * s.getSite_range_place()); document.put("site_other_line",
	 * s.getSite_other_line()); document.put("site_other_place",
	 * s.getSite_other_place()); document.put("site_machineRoomCode",
	 * s.getSite_machineRoomCode()); document.put("assetOwnership",
	 * s.getAssetOwnership()); document.put("ownershipUnitName",
	 * s.getOwnershipUnitName()); document.put("ownershipUnitCode",
	 * s.getOwnershipUnitCode()); document.put("maintainBody", s.getMaintainBody());
	 * document.put("maintainUnitName", s.getMaintainUnitName());
	 * document.put("maintainUnitCode", s.getMaintainUnitCode());
	 * document.put("manufacturers", s.getManufacturers());
	 * document.put("deviceType", s.getDeviceType()); document.put("useUnit",
	 * s.getUseUnit()); document.put("totalCapacity", s.getTotalCapacity());
	 * document.put("roadCapacity", s.getRoadCapacity()); document.put("assetRatio",
	 * s.getAssetRatio()); document.put("productionDate", s.getProductionDate());
	 * document.put("useDate", s.getUseDate());
	 * document.put("deviceOperationStatus", s.getDeviceOperationStatus());
	 * document.put("stopDate", s.getStopDate()); document.put("scrapDate",
	 * s.getScrapDate()); document.put("fixedAssetsCode", s.getFixedAssetsCode());
	 * document.put("remark", s.getRemark());
	 * 
	 * document.put("userId", userId); document.put("orgId", orgId);
	 * this.addDocument(document,collectionName); }); // //删除数据库中来源为上一次导入的数据 //
	 * service.removeDocument(reatList, "deviceRecord"); return
	 * ResultMsg.getSuccess("导入成功"); } }
	 */
