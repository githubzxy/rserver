...
define('yearMonthPlan/yearMonthPuTie/compile/WorkAreaPuTieYearMonth',[
	...
	'yearMonthPlan/yearMonthPuTie/compile/WorkAreaPuTieYearMonthImport',
	],function(require){
	...
	WorkAreaPuTieYearMonthImport = require('yearMonthPlan/yearMonthPuTie/compile/WorkAreaPuTieYearMonthImport');
	...
	var WorkAreaPuTieYearMonth = Component.Controller.extend({
		...
		bindUI : function() {
			...
			//'导入' 链接事件
			table.on("itemclick",function(e){
				_self._import(e,tbar,store);
			});
		},
		
		_import : function(e,tbar,store){
			var _self = this;
			var record = e.item;
			var target = $(e.domTarget);
//			console.log(e);
			if(target.hasClass('import')){
				var ImportForm = new WorkAreaPuTieYearMonthImport({
					user : _self.get('user'),
					id : record.id,
					name : record.name,
					year : record.year,
		...
		_initColumns : function(){
			...
			var columns = [{
				...
				renderer :function(value,obj){
					if(obj.attachPath8_1==null||obj.attachPath8_2==null){
						return "导入表阶段";
					}
...
//					console.log(obj);
					...
					if(obj.attachPath8_1==null&&obj.attachPath8_2==null){
						return '<span id="import" class="grid-command import">导入</span>'
					}
