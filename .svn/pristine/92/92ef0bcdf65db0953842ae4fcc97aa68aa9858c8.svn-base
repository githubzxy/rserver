 ...
 * 导入施工维修信息表单
 */
define('kmms/communicationDevice/deviceRecordLedger/deviceRecordLedgerImport',
[
										'bui/overlay',
										'bui/mask',
	...
	var deviceRecordLedgerImport = Overlay.Dialog.extend({
		//初始化弹框控件
...
//        		}
		...
		ATTRS : {//定义弹框属性
			elAttrs : {value: {id:"deviceRecordLedgerImportDlog"}},
			title : {value : '导入检修日志表'},
			width : {value : 617},
//			collectionName:{},
			height : {value : 170},
			...
			success : {
				value : function(){
					...
					var importForm=_self.getChild('deviceRecordLedgerImport',true);
...
//	                    formData.append('userId',_self.get('userId'));
	                    ...
	                    xhr.open("POST", "/kmms/deviceRecordLedgerAction/Import");
	                    xhr.send(formData);
	...
	return deviceRecordLedgerImport;
});