...
@Service("communicationNetworkSkillAuditService")
public class CommunicationNetworkSkillAuditServiceImpl implements  CommunicationNetworkSkillAuditService{
...
//        filter = Filters.and(filter,Filters.eq("parentId",orgId));//表示查询的是当前登陆用户所属组织机构下的所有机构的数据
        if(orgId.equals("402891b45b5fd02c015b74ca35180039")){//技术科id
        filter = Filters.and(filter,Filters.in("flowState","1"));//表示可以查询到除了草稿状态以外的数据
        }else{
