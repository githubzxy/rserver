...
@Service(value = "JobRecordQueryService")
public class JobRecordQueryServiceImpl implements JobRecordQueryService{
...
//        }
		...
		*/
		...
		private Bson getFilter(String workshopType,String workshopId,String workareaId, String startDate,String endDate, String project, String orgId, int orgType) {
			...
			filter = Filters.and(filter, Filters.eq("filesStatus", "1"));
...
//	        }
		...
		/*
		 * Cell cellSecond = rowSec.getCell(i*4+2);//表头
		 * 
		 * cellSecond.setCellValue(workArea); for (int a = 0; a <jobRecordDtos.size();
		 * a++) { Row row = s.getRow(a+3); Cell cell2 = row.getCell(i*4+2); Cell cell3 =
		 * row.getCell(i*4+3); Cell cell4 = row.getCell(i*4+4); Cell cell5 =
		 * row.getCell(i*4+5);
		 * cell2.setCellValue(jobRecordDtos.get(a).getJoinerNumber());
		 * cell3.setCellValue(jobRecordDtos.get(a).getDirectHour());
		 * cell4.setCellValue(jobRecordDtos.get(a).getOtherHour());
		 * cell5.setCellValue(jobRecordDtos.get(a).getAllHour()); }
		 */
			return wb;
		}
		private HSSFWorkbook writeDataInExcel(String date, String workShop, String workArea,
		 ...
		 * font.setFontHeightInPoints((short) 20); cellStyle.setFont(font);
		 * cellStyle.setAlignment(HSSFCellStyle.ALIGN_CENTER);//居中
		 */		
			cell1.setCellValue(titleString);
			cellSecond.setCellValue(workArea);
