...
@Service(value = "visitRoomRecordsService")
public class VisitRoomRecordsServiceImpl implements VisitRoomRecordsService{
	...
	private Bson getFilter(String name, String machineRoom,String  railLine,String startDate,String endDate,String currentDay,String userId) {
		...
		if (StringUtils.isNotBlank(name)) {
			filters = Filters.and(filters,Filters.regex("helper", name));
		}
	...
	public String getLineDataId(String railLine) {
		String sql="select rail_line_id_ from res_base_rail_line  where name_ = :railLine";
		HashMap<String ,Object> paramMap = new HashMap<String ,Object>();
		paramMap.put("railLine", railLine);
		String lineId=template.queryForObject(sql, paramMap, String.class);
		return lineId;
	}

	@Override
	public String getmachineRoomDataId(String machineRoom) {
		String sql="select id_ from res_base_rail_machineroom  where machine_name_= :machineRoom";
		HashMap<String ,Object> paramMap = new HashMap<String ,Object>();
		paramMap.put("machineRoom", machineRoom);
		String machineRoomId=template.queryForObject(sql, paramMap, String.class);
