 ...
 * @date 19-06-17
 */
define('kmms/technicalManagement/communicationResumeManage/wireless/handEdit',['bui/common',
    ...
    var IrontowerEdit = BUI.Overlay.Dialog.extend({
            ...
            var childs = [
                {
                    ...
                    redStarFlag : true,
                    itemColspan : 1,
                    item : '<input type="text" name="site_department" id="site_department" style="width:99%" data-rules="{required:true}"/>'
                },{
                    label : '资产归属：',
                    redStarFlag : true,
                    itemColspan : 1,
        ...
        ATTRS : {
            id : {value : 'handEditDialog'},
            elAttrs : {value: {id:"handPointEdit"}},
            title:{value:'编辑手持设备-06'},
            width:{value:630},
            height:{value:500},
            contextPath : {},
            ...
            success:{
                value : function(){
                    var _self = this;
                    var formAdd = _self.getChild('handEditForm'),delData=_self.get('delData');
                    //验证不通过
	        		if(!formAdd.isValid()){
	        			return;
	        		}
                    ...
                    xhr.open("POST", "/zuul/kmms/wirelessAction/updateDocHand");
                    xhr.send(formData);
