        ...
        _initFormContainer : function(){
            ...
            var childs = [
                ...
                },
                {
                	label : '备注：',
                	itemColspan : 2,
                	item : '<textarea name="approveRemark" id="approveRemark" style="width:99.5%;height:150px" /></textarea>'
                }
                 ];
        ...
        ATTRS : {
            ...
            success:{//会改变流程状态，若内容也修改了的话一并更新
                value : function(){
                    ...
                    formData.append('uploadFileArr',JSON.stringify(uploadFileArr));
                    var xhr = new XMLHttpRequest();
                    xhr.open("POST", "/zuul/kmms/pointOuterMaintainApproveAction/updateDocAndContent");
                    xhr.send(formData);
                    xhr.onload = function (e) {
                        if (e.target.response) {
                            var result = JSON.parse(e.target.response);
