 ...
 * @date 19-06-17
 */
define('kmms/technicalManagement/communicationResumeManage/wireless/radioEdit',['bui/common',
	'common/form/FormContainer',
    'bui/form',
    ],function(r){
    var BUI = r('bui/common'),
        FormContainer= r('common/form/FormContainer');
    var RadioEdit = BUI.Overlay.Dialog.extend({
            ...
            $.ajax({
                ...
                success:function(res){
                    ...
                    if(data){
                        ...
                        $('#formContainer #phaseAlternatingLine').val(data.phaseAlternatingLine);
                        $('#formContainer #quadNumber').val(data.quadNumber);
                        $('#formContainer #grandTrine').val(data.grandTrine);
                        $('#formContainer #trainNumberCheck').val(data.trainNumberCheck);
                        $('#formContainer #dispatchCommand').val(data.dispatchCommand);
            ...
            var childs = [
                {
                    ...
                    redStarFlag : true,
                    itemColspan : 1,
                    item : '<input type="text" name="phaseAlternatingLine" id="phaseAlternatingLine" style="width:98.5%" data-rules="{required:true}"/>'
                },{
                    label : '四频组序号：',
                    redStarFlag : true,
                    itemColspan : 1,
                    item : '<input type="text" name="quadNumber" id="quadNumber" style="width:99%" data-rules="{required:true}"/>'
                },{
                    label : '大三角：',
                    redStarFlag : true,
                    itemColspan : 1,
                    item : '<input type="text" name="grandTrine" id="grandTrine" style="width:99%" data-rules="{required:true}"/>'
                },{
                    label : '车次号校验：',
                    redStarFlag : true,
                    itemColspan : 1,
                    item : '<input type="text" name="trainNumberCheck" id="trainNumberCheck" style="width:99%" data-rules="{required:true}"/>'
                },{
                    label : '调度命令传送：',
                    redStarFlag : true,
                    itemColspan : 1,
                    item : '<input type="text" name="dispatchCommand" id="dispatchCommand" style="width:98.5%" data-rules="{required:true}"/>'
                },{
                    label : '设备厂家：',
                    redStarFlag : true,
                    itemColspan : 1,
        ...
        ATTRS : {
            id : {value : 'radioEditDialog'},
            elAttrs : {value: {id:"radioPointEdit"}},
            title:{value:'编辑车站电台-03'},
            width:{value:630},
            height:{value:500},
            contextPath : {},
            ...
            success:{
                value : function(){
                    var _self = this;
                    var formAdd = _self.getChild('radioEditForm'),delData=_self.get('delData');
                    //验证不通过
	        		if(!formAdd.isValid()){
	        			return;
	        		}
                    ...
                    xhr.open("POST", "/zuul/kmms/wirelessAction/updateDocRadio");
                    xhr.send(formData);
