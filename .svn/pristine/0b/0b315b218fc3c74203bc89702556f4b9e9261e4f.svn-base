...
@Service("communicationNetworkSafeAuditService")
public class CommunicationNetworkSafeAuditServiceImpl implements  CommunicationNetworkSafeAuditService{
...
//        filter = Filters.and(filter,Filters.eq("parentId",orgId));//表示查询的是当前登陆用户所属组织机构下的所有机构的数据
        if(orgId.equals("402891b45b5fd02c015b74a20fd10033")){//安全科组织id
            filter = Filters.and(filter,Filters.in("flowState","2","3"));//表示可以查询技术科审核通过后的数据
            }else{
