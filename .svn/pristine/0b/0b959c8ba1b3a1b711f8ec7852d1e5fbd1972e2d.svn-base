 ...
 * @date 19-6-19
 */
define('kmms/technicalManagement/communicationResumeManage/dispatch/stationChangerEdit',['bui/common',
	'common/form/FormContainer',
    'bui/form',
    ],function(r){
    var BUI = r('bui/common'),
        FormContainer= r('common/form/FormContainer');
    var stationChangerEdit = BUI.Overlay.Dialog.extend({
...
//        },
            ...
            $.ajax({
                ...
                success:function(res){
                    ...
                    if(data){
            		    ...
            		    $('#formContainer #numOfTrunkConf').val(data.numOfTrunkConf);
            		    $('#formContainer #numOfTrunkUsed').val(data.numOfTrunkUsed);
            		    $('#formContainer #numOfUserConf').val(data.numOfUserConf);
            		    $('#formContainer #numOfUserUsed').val(data.numOfUserUsed);
            ...
            var childs = [
            	{
                    ...
                    redStarFlag : true,
                    itemColspan : 1,
                    item : '<input type="text" name="userNumber" id="userNumber" style="width:99%" data-rules="{required:true}"/>'
                },{
                    label : '设备厂家：',
                    redStarFlag : true,
                    itemColspan : 1,
                ...
                },
                {
                    ...
                    redStarFlag : true,
                    itemColspan : 1,
                    item : '<input type="text" name="numOfTrunkConf" id="numOfTrunkConf" style="width:98.5%" data-rules="{required:true}"/>'
                },
                {
                    label : '2M端口实际使用数量：',
                    redStarFlag : true,
                    itemColspan : 1,
                    item : '<input type="text" name="numOfTrunkUsed" id="numOfTrunkUsed" style="width:98.5%" data-rules="{required:true}"/>'
                },
                {
                    label : '值班台数量：',
                    redStarFlag : true,
                    itemColspan : 1,
                    item : '<input type="text" name="numOfUserConf" id="numOfUserConf" style="width:98.5%" data-rules="{required:true}"/>'
                },
                {
                    label : '电话分机数量：',
                    redStarFlag : true,
                    itemColspan : 1,
                    item : '<input type="text" name="numOfUserUsed" id="numOfUserUsed" style="width:98.5%" data-rules="{required:true}"/>'
                },
                {
                    label : '固资编号：',
                    itemColspan : 2,
        ...
        ATTRS : {
            id : {value : 'stationChangerEditDialog'},
            elAttrs : {value: {id:"stationChangerPointEdit"}},
            title:{value:'干线调度-01'},
            width:{value:630},
            height:{value:500},
            contextPath : {},
            ...
            success:{
                value : function(){
                    var _self = this;
                    var formAdd = _self.getChild('stationChangerEditForm'),delData=_self.get('delData');
                    //验证不通过
	        		if(!formAdd.isValid()){
	        			return;
	        		}
                    ...
                    xhr.open("POST", "/zuul/kmms/dispatchAction/stationChanger/updateDoc");
                    xhr.send(formData);
